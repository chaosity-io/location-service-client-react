...
exports.snapToRoadsResponseToFeatureCollection = exports.optimizeWaypointsResponseToFeatureCollection = exports.calculateIsolinesResponseToFeatureCollection = exports.calculateRoutesResponseToFeatureCollections = void 0;
const utils_1 = require("./utils");
const polyline_1 = require("@aws/polyline");
const defaultBaseGeoRoutesOptions = {
    flattenProperties: true,
};
const defaultCalculateRoutesResponseOptions = Object.assign(Object.assign({}, defaultBaseGeoRoutesOptions), { includeLegs: true, includeTravelStepGeometry: false, includeSpans: false, includeLegArrivalDeparturePositions: false, includeTravelStepStartPositions: false });
/**
 ...
 * ```
 */
function calculateRoutesResponseToFeatureCollections(routesResponse, options) {
    const routes = [];
    // Any options that weren't passed in will get set to default values.
    options = Object.assign(Object.assign({}, defaultCalculateRoutesResponseOptions), options);
        ...
        for (const leg of route.Legs) {
            if (leg.Geometry) {
                ...
                if (options.includeLegs) {
                    ...
                    const { Geometry } = leg, legProperties = __rest(leg, ["Geometry"]);
                ...
                if (options.includeTravelStepGeometry) {
                    extractNestedLinesIntoCollection(routeCollection, "TravelStepGeometry", legLineString, legDetails === null || legDetails === void 0 ? void 0 : legDetails.TravelSteps, options.flattenProperties);
                }
                ...
                if (options.includeTravelStepStartPositions) {
                    extractNestedPointsIntoCollection(routeCollection, "TravelStepStartPosition", legLineString, legDetails === null || legDetails === void 0 ? void 0 : legDetails.TravelSteps, options.flattenProperties);
                }
                ...
                if (options.includeSpans) {
                    extractNestedLinesIntoCollection(routeCollection, "Span", legLineString, legDetails === null || legDetails === void 0 ? void 0 : legDetails.Spans, options.flattenProperties);
                }
            }
        }
...
exports.calculateRoutesResponseToFeatureCollections = calculateRoutesResponseToFeatureCollections;
 ...
 * ```
 */
function calculateIsolinesResponseToFeatureCollection(isolinesResponse, options) {
    // Set any options that weren't passed in to the default values.
    options = Object.assign(Object.assign({}, defaultCalculateIsolinesResponseOptions), options);
    const isolines = {
        type: "FeatureCollection",
        features: [],
    };
    ...
    for (const isoline of isolinesResponse.Isolines) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { Geometries, Connections } = isoline, properties = __rest(isoline, ["Geometries", "Connections"]);
        const feature = {
            type: "Feature",
            id: isolines.features.length,
            properties: options.flattenProperties ? (0, utils_1.flattenProperties)(properties, "") : properties,
            geometry: {
        ...
        if (feature.geometry.geometries.length > 0) {
            if (feature.geometry.geometries.length === 1 && feature.geometry.geometries[0].type === "Polygon") {
                ...
                // between those areas
                isolines.features.push(Object.assign(Object.assign({}, feature), { geometry: feature.geometry.geometries[0] }));
            }
            else {
...
exports.calculateIsolinesResponseToFeatureCollection = calculateIsolinesResponseToFeatureCollection;
 ...
 * ```
 */
function optimizeWaypointsResponseToFeatureCollection(waypointsResponse, options) {
    const waypoints = {
        type: "FeatureCollection",
        features: [],
    };
    // Set any options that weren't passed in to the default values.
    options = Object.assign(Object.assign({}, defaultOptimizeWaypointsResponseOptions), options);
    ...
    for (const impedingWaypoint of waypointsResponse.ImpedingWaypoints) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { Position } = impedingWaypoint, properties = __rest(impedingWaypoint, ["Position"]);
    ...
    for (const optimizedWaypoint of waypointsResponse.OptimizedWaypoints) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { Position } = optimizedWaypoint, properties = __rest(optimizedWaypoint, ["Position"]);
...
exports.optimizeWaypointsResponseToFeatureCollection = optimizeWaypointsResponseToFeatureCollection;
const defaultSnapToRoadsResponseOptions = Object.assign(Object.assign({}, defaultBaseGeoRoutesOptions), { includeSnappedGeometry: true, includeSnappedTracePointOriginalPositions: false, includeSnappedTracePointSnappedPositions: false, includeOriginalToSnappedPositionLines: false });
/**
 ...
 * ```
 */
function snapToRoadsResponseToFeatureCollection(snapToRoadsResponse, options) {
    // Set any options that weren't passed in to the default values.
    options = Object.assign(Object.assign({}, defaultSnapToRoadsResponseOptions), options);
    ...
    if (options.includeSnappedGeometry) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { SnappedGeometry, SnappedGeometryFormat, SnappedTracePoints } = snapToRoadsResponse, properties = __rest(snapToRoadsResponse, ["SnappedGeometry", "SnappedGeometryFormat", "SnappedTracePoints"]);
        properties["FeatureType"] = "SnappedGeometry";
        delete properties["$metadata"];
        addFeatureToCollection(snappedFeatures, options.flattenProperties, properties, createLineString(snapToRoadsResponse.SnappedGeometry));
    }
    for (const snappedTracePoint of snapToRoadsResponse.SnappedTracePoints) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { OriginalPosition, SnappedPosition } = snappedTracePoint, tracePointProperties = __rest(snappedTracePoint, ["OriginalPosition", "SnappedPosition"]);
        // Optionally include a Point for each original TracePoint submitted.
        if (options.includeSnappedTracePointOriginalPositions) {
            const originalProperties = Object.assign({}, tracePointProperties);
        ...
        if (options.includeSnappedTracePointSnappedPositions) {
            const snappedProperties = Object.assign({}, tracePointProperties);
        ...
        if (options.includeOriginalToSnappedPositionLines) {
            const snapLineProperties = Object.assign({}, tracePointProperties);
...
exports.snapToRoadsResponseToFeatureCollection = snapToRoadsResponseToFeatureCollection;
/**
 ...
 */
function addFeatureToCollection(collection, flatten, properties, geometry) {
    collection.features.push({
        type: "Feature",
        id: collection.features.length,
        properties: flatten ? (0, utils_1.flattenProperties)(properties, "") : properties,
        geometry: geometry,
    });
}
/**
 ...
 */
function createLineString(geometry) {
    if (geometry.LineString) {
        ...
        return (0, polyline_1.decodeToLineString)(geometry.Polyline);
    }
    else {
...
function createPolygon(geometry) {
    if (geometry.Polygon) {
        ...
        return (0, polyline_1.decodeToPolygon)(geometry.PolylinePolygon);
    }
    else {
 ...
 */
function extractNestedLinesIntoCollection(features, lineFeatureType, lineString, featureList, flattenProperties) {
    if (!featureList)
        return;
    for (const [index, feature] of featureList.entries()) {
        ...
        const stepLineString = Object.assign({}, lineString);
        stepLineString.coordinates = lineString.coordinates.slice(feature["GeometryOffset"], index < featureList.length - 1 ? featureList[index + 1]["GeometryOffset"] + 1 : lineString.coordinates.length);
        ...
        if (stepLineString.coordinates.length > 1) {
            ...
            const featureProperties = Object.assign({}, feature);
 ...
 * @param flattenProperties Whether to flatten the properties or not.
 */
function extractNestedPointsIntoCollection(features, startPointFeatureType, lineString, featureList, flattenProperties) {
    if (!featureList)
        return;
    for (const [index, feature] of featureList.entries()) {
        // The featureList array contains an array of entries that each contain a GeometryOffset. The
