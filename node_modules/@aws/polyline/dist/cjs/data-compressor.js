...
class DataCompressor {
    ...
    polygonIsCounterClockwise(lngLatArray) {
    ...
    positionsAreEquivalent(pos1, pos2) {
    ...
    decodeLineString(compressedData) {
        const [decodedLine, compressionParameters] = this.decompressLngLatArray(compressedData);
        ...
        if (decodedLine.length < 2) {
            throw Error(`Invalid LineString. LineStrings must contain 2 or more positions. It contains ${decodedLine.length} positions.`);
        }
        return [
            {
                type: "LineString",
    ...
    decodePolygon(compressedData) {
        ...
        for (const ring of compressedData) {
            const [decodedRing, ringCompressionParameters] = this.decompressLngLatArray(ring);
            ...
            if (decodedRing.length < 4) {
                throw Error(`Invalid polygon. Polygons must contain 4 or more positions. It contains ${decodedRing.length} positions. Consider decoding as a LineString.`);
            }
            ...
            if (!this.positionsAreEquivalent(decodedRing[0], decodedRing[decodedRing.length - 1])) {
                throw Error(`Invalid polygon. The first and last positions must contain identical values. The values are ${decodedRing[0]}, ${decodedRing.at(-1)}. Consider decoding as a LineString.`);
            }
            ...
            // validating it.
            if (shouldBeCounterclockwise != this.polygonIsCounterClockwise(decodedRing)) {
                decodedRing.reverse();
            }
    ...
    compressionParametersToGeoJsonProperties(parameters) {
        switch (parameters.thirdDimension) {
            case polyline_types_1.ThirdDimension.Level:
                return {
                    precision: parameters.precisionLngLat,
                    thirdDimensionPrecision: parameters.precisionThirdDimension,
                    thirdDimensionType: "level",
                };
            case polyline_types_1.ThirdDimension.Elevation:
                return {
                    precision: parameters.precisionLngLat,
                    thirdDimensionPrecision: parameters.precisionThirdDimension,
                    thirdDimensionType: "elevation",
                };
            case polyline_types_1.ThirdDimension.Altitude:
                return {
                    precision: parameters.precisionLngLat,
                    thirdDimensionPrecision: parameters.precisionThirdDimension,
    ...
    encodeFromLngLatArray(lngLatArray, parameters) {
        return this.compressLngLatArray(lngLatArray, parameters);
    }
    decodeToLngLatArray(compressedData) {
        const [decodedLngLatArray] = this.decompressLngLatArray(compressedData);
        return decodedLngLatArray;
    }
    decodeToLineString(compressedData) {
        const [lineString] = this.decodeLineString(compressedData);
        return lineString;
    }
    decodeToPolygon(compressedData) {
        const [polygon] = this.decodePolygon(compressedData);
        return polygon;
    }
    decodeToLineStringFeature(compressedData) {
        const [lineString, compressionParameters] = this.decodeLineString(compressedData);
        return {
            type: "Feature",
            geometry: lineString,
            properties: this.compressionParametersToGeoJsonProperties(compressionParameters),
        };
    }
    decodeToPolygonFeature(compressedData) {
