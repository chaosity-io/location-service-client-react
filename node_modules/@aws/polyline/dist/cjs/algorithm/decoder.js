...
exports.PolylineDecoder = void 0;
...
class PolylineDecoder {
    constructor(decodingTable, containsHeader) {
    ...
    decodeSignedValue(encoded, startIndex) {
        // decode an unsigned value
        const [unsignedValue, nextIndex] = this.decodeUnsignedValue(encoded, startIndex);
    ...
    decodeUnsignedValue(encoded, startIndex) {
        let result = 0;
        let shift = 0;
        ...
        while (index < encoded.length) {
            ...
            if (value < 0) {
                throw Error(`Invalid input. Encoded character '${charCode}' doesn't exist in the decoding table.`);
            }
        ...
        throw Error("Invalid encoding - last block contained an extra 0x20 'continue' bit.");
    }
    decodeHeader(encoded) {
        ...
        if (headerVersion !== polyline_types_1.FlexiblePolylineFormatVersion) {
            throw new Error("Invalid format version");
        }
        const [metadata, nextIndex] = this.decodeUnsignedValue(encoded, metadataIndex);
        return [
            {
    ...
    decode(encoded, encodePrecision = 0) {
        ...
        while (index < encoded.length) {
            ...
            for (let dimension = 0; dimension < numDimensions; dimension += 1) {
                ...
                // digits of precision.
                const value = lastScaledCoordinate[dimension] / precisionDivisors[dimension];
                if (Math.abs(value) > maxAllowedValues[dimension]) {
                    throw Error(`Invalid input. Compressed data contains invalid coordinate value: ${value}`);
                }
...
exports.PolylineDecoder = PolylineDecoder;
