...
exports.PolylineEncoder = void 0;
...
class PolylineEncoder {
    constructor(encodingTable, includeHeader) {
    ...
    encode(lngLatArray, precision, thirdDim = polyline_types_1.ThirdDimension.None, thirdDimPrecision = 0) {
        if (precision < 0 || precision > 11) {
            throw Error("Only precision values of 0-11 decimal digits are supported.");
        }
        if (!Object.values(polyline_types_1.ThirdDimension).includes(+thirdDim)) {
            throw Error("thirdDim is an invalid ThirdDimension value.");
        }
        if (thirdDimPrecision < 0 || thirdDimPrecision > 11) {
            throw Error("Only thirdDimPrecision values of 0-11 decimal digits are supported.");
        }
        ...
        lngLatArray.forEach((coordinate) => {
            if (coordinate.length != numDimensions) {
                throw Error("Invalid input. All coordinates need to have the same number of dimensions.");
            }
            for (let dimension = 0; dimension < numDimensions; dimension++) {
                ...
                if (Math.abs(inputValue) > maxAllowedValues[dimension]) {
                    throw Error(`Invalid input. Input coordinates must contain valid lng/lat coordinate data. Found ${coordinate}.`);
                }
                ...
                // the next delta.
                const scaledValue = this.polylineRound(inputValue * precisionMultipliers[dimension]);
                output += this.encodeSignedValue(scaledValue - lastScaledCoordinate[dimension]);
                lastScaledCoordinate[dimension] = scaledValue;
            }
        });
        return output;
    }
    encodeHeader(precision, thirdDim, thirdDimPrecision) {
        // Combine all the metadata about the encoded data into a single value for the header.
        const metadataValue = (thirdDimPrecision << 7) | (thirdDim << 4) | precision;
        return (this.encodeUnsignedValue(polyline_types_1.FlexiblePolylineFormatVersion) +
